...
```bash
untitled1$ aws s3 mb s3://BUCKET_NAME
```

To prepare the application for deployment, use the `sam package` command.

```bash
untitled1$ sam package \
...
```bash
untitled1$ sam deploy \
    --template-file packaged.yaml \
    --stack-name untitled1 \
...
```bash
untitled1$ cd HelloWorldFunction
...
```bash
untitled1$ sam delete --stack-name untitled1
untitled1$ aws s3 rb s3://BUCKET_NAME
```
